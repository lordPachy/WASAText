...
func (db *appdbimpl) SendMessage(newMessage Message) {
	message_query := "('" + strconv.Itoa(newMessage.MessageID) + "', '" + newMessage.Username + "', '" + newMessage.Timestamp + "', '" + newMessage.Content + "', '" + newMessage.Photo + "', '" + strconv.Itoa(newMessage.Checkmarks) + "', '" + strconv.Itoa(newMessage.ReplyingTo) + "');"
